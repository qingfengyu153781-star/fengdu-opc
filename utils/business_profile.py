...
_SPECIAL_PARSERS = {
    "region": [
        ...
        # 兼容"我是重庆的，做直播带货"（城市词后有"的"+逗号再跟动词，逗号隔断时允许"的"与标点跳过）
        # "做/搞/从事"前无字（"我是做摄影的"）→ 不匹配，绝不猜错
        (r"我(?:是|在)?([一-龥]{2,8}?)(?:的)?[,，、\s]*(?:做|搞|从事|干|开|经营)([一-龥]{2,10})",
         lambda m: m.group(1)),
    ],
    "reg_type": [
...
def extract_industry_free(text: str) -> str:
    ...
    # 兼容"开了2年摄影工作室""做了3年软件开发"（动词后带时长再跟行业）：
    # 动词后允许「了/着/过」「N年/N个月/半年」「的/家」再抓行业词（至少 2 字）
    m = re.search(r"(?:我|本人)?(?:做|从事|搞|干|开|经营|主做|主营)"
                  r"(?:了|着|过)?(?:[0-9一二两三四五六七八九十]+(?:年|个月|年半)?)?(?:的|家)?([一-龥]{2,8})", t)
    if m:
        ind = m.group(1).strip()
        # 去掉"的/了/行业/公司"等词尾
