...
## 十四、质量审查记录（2026-08-06 三轮深度审查+修复）

> 评审视角审查了 PPT 16 页 / demo 全部代码 / 部署清单，发现并修复 10 项问题（详见 CLAUDE.md 铁律与 `docs/token-waste-postmortems.md#20`）。

| # | 问题 | 修复 |
|---|------|------|
| F1 | 行业抽不到："开了2年摄影工作室"（时长在动词后）→ 行业空 → 反复追问同一问题 8 轮 | `extract_industry_free` 支持"动词+了+N年+行业" |
| F2 | 团队规模误填：被问团队人数时答"单客户60%"→ 60 塞进 team_size → 预审提前完成 | `process_chat` 追问兜底：已识别其他字段时不硬塞数字 |
| F3 | 地区识别失败："我是重庆的，做直播带货"（有逗号）→ 重庆抽不到 | region 正则容忍"的+逗号+动词" |
| F4 | 地区误判："客户比较分散"被直录成地区 → 政策路由走错 | region 直录只接受纯地名 |
| F5 | 材料清单误报：对话报告"✅材料完整"实际 19 待确认 | 报告按缺失/待确认实际措辞 |
| F6 | has_materials 无解析："有营业执照"不被记录 | 新增材料抽取 |
| F7 | PPT 三处夸大当现状（未来预测/加权/偏见过滤） | 改"当下可解释/5维等权20%/规则综合判定" |
| F8 | P13 健康公式与代码不符（0.25/0.15 算不出 72） | 统一 5 维等权 20% |
| F9 | P7 政策标签非实测结果 | 改实测"市级/区级创业补贴+人才驿站" |
| F10 | 测试污染 `custom_region.py` 进 commit | 撤销 commit、删除、重提交 |

**开发约束（避免复发）：**
- 改 `make_ppt.py` 后必须重生成验证（它=PPT 真相源）；直接改 PPTX 必须同步 make_ppt.py
- 测试 `import_policy` 类写文件函数时隔离，禁止污染 `policies/`；commit 前 `git status`
- 演示脚本/文档任何"输出断言"（符合 N 项/健康 72）先 `python` 实测再写

---

*开发文档 v1.0 · 与代码同步：2026-08-06 · 质量审查记录追加*
